...
# VIFs are allocated on demand for interfaces named below; the daemon
# no longer enumerates every multicast-capable interface up front.
# Use `phyint` only when you want to override the default TTL threshold
# or enable mrdisc; an mroute or mgroup line is enough by itself.
...
# An mroute whose inbound or outbound interface does not yet exist is
# kept on a pending list rather than dropped.  Useful for tunnels that
# come up after smcrouted starts (WireGuard, 6LoWPAN, etc.); the route
# is activated when the interface arrives.  Inspect the queue with
